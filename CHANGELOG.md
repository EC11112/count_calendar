...
- `.cal-cell-month` 字号从 `badgeFont`（cap 8-10px）改为 `badgeFont × 2`（16-20px）
- 用户反馈 v0.3.18 的月份名偏小，要求放大到 200%
- 定位逻辑不变：仍在左下角，格子够大时抬到徽章上方
...
- 场景：A 半天 + B 半天 = 1.0 已满，C 是当前活动项目已是半天
- 用户操作：点击 9/14 想加整天
- 期望：把"加整天被堵"理解为"想覆盖当前半天"，自动取消 C 的半天
- 实际（v0.3.16）：弹红色 warn toast「无法再添整天」,死锁,用户无法操作

### 2. 根因

- `getDayHoursTotal` 排除了当前项目（用来算"再加多少会超限"）
- 当 `otherTotal + newValue > 1.0` 直接返回 warn toast
- 但 `cur === 1 && next === 2` 时,用户的本意可能不是"加整天",而是"想调整 C 的状态"
- 撞墙后没有 fallback → 用户死锁

### 3. 修复

- 在原有校验前加一个**特殊场景判断**:
  - `cur === 1 && next === 2 && otherTotal === 0.5`
  - 即"当前是半天,想点整天,但其他项目合计正好 0.5 → 当天已满"
- 命中时直接 `setClicks(C, 0)` 清掉当前半天
- **不需要用户再点一次**才能取消 → 解锁
- toast 提示 `当前项目已取消半天 (其他项目已占满一天)` **暂时注释掉**——用户反馈"加个 toast 反而打扰",先静默取消,后续按需打开

### 4. 不影响其他场景

- 第一次点（C 0→1）: cur=0 next=1 → 走主校验 (0.5+0.5=1.0 ≤ 1.0 通过) ✓
- 第二次点 C 加整天撞墙: 走特殊场景 → 自动清 C ✓
- 第三次点（C 现在是 0）: cur=0 next=1 → 走主校验 (0.5+0.5=1.0 OK) ✓ — 可以正常再加半天
- 整体行为: 撞墙后 C 自动清掉,用户能继续操作,没有任何死锁场景

### 5. APP_VERSION 0.3.16 → 0.3.17
...
### 1. 界面按钮 icon 全部走黑白线性格式

- 顶部 stats panel 4 个 icon 按钮：✎ ? ⚙（之前部分系统是彩的）→ 全统一成黑白线性
- sidebar 底部三按钮：+ ✎ 已经是黑白，新增的 `×` 清除数据按钮也走黑白
- 字符映射：
  - 顶部下载按钮：`📥` → **`↓`** (U+2193 向下箭头，纯符号,不是 emoji)
  - sidebar 清除数据：`🗑` → **`×`** (U+00D7 乘号，ASCII 字符,绝对黑白)
  - `⚙` 字符本身在 emoji 范围，靠 CSS 强制线性格式（见下）
  - `✎` `?` `+` 已经是黑白，保留不动

### 2. CSS 防御：`font-variant-emoji: text`

- 新增 CSS 规则作用于 `.stats-actions .header-icon-btn` 和 `.sidebar-bottom .btn-icon`
- 强制所有按钮 icon 走文本字体，不让浏览器把 `⚙` 等字符按 emoji 渲染成彩色
- Chrome 116+ / Firefox / Safari 16.4+ 支持
- 旧浏览器 / 不支持时,按钮里的 icon 字符本身是黑白 BMP 字符,自然就是黑白的（双保险）

### 3. 欢迎页 / 介绍文案保留彩色 emoji

- 欢迎页 6 个 section 标题：`📅 记录` `👁 视图与统计` `🎯 项目管理` `✎ 编辑模式` `🗑 清除数据` `⚙ 设置与数据` —— 全部保留彩色
- 欢迎页正文里的 `👁` `🗑` `⚙` —— 保留彩色
- 项目列表全隐藏时的提示文本 `显示一个看看 👁` —— 保留彩色
- 确认对话框文本 `建议先在 ⚙ 里导出备份` —— 保留彩色
- **理由**：欢迎页/提示文案是叙述性文字，emoji 起视觉锚点作用，彩色更直观
- 界面按钮 vs 文案 emoji 分工：按钮=控制元素（统一线性），文案=解释元素（保留彩色温度）

### 4. 不动的位置

- 代码注释里的 emoji（`📥` `🗑` `🛠` 等）—— 看不到,保留
- 字符常量、`COLORS` 数组等无 UI 影响的地方 —— 不动
...
- **白色 = 中性默认**：`+ 新建项目` 和 `✏ 编辑项目` 默认都是白底灰边框，跟默认 `.btn` 样式一致
- **蓝色 = 当前态/主操作**：点击 `✏ 编辑项目` 后按钮变 `✕ 退出编辑` + 蓝色填充 (`.btn-primary`) —— 表示"当前正在编辑"
- **红色 = 危险操作**：`🗑 清除数据` 红色填充 (`.btn-danger`) —— 跟蓝色的"当前态"区分开
- 三色分工清晰：白色中性 / 蓝色当前 / 红色危险
...
- **统一铅笔图标** ✏ → ✎（斜的更明显是铅笔）
  - 顶部 stats 面板的 ✎ 按钮、sidebar 底部 ✎ 编辑项目按钮、欢迎页文案、toast 提示、项目编辑模式注释
  - 跟项目列表的 ✎ 编辑按钮保持一致
- **设置菜单"星期数"** → **"最大显示星期"**（更准确，避免歧义）
  - 季/年视图 tooltip 里也同步改用"最大显示星期"
- **tooltip/文案审计修复**：
  - 导航按钮 placeholder "上一段/下一段" → "向前/向后"（之前"段"字莫名其妙）
  - 顶部 ✎ 按钮 title 补上非工时类说明（"工时类 0→半天→整天，非工时类 0→√"），之前只说工时类
  - 欢迎页"记录"section 补上非工时类说明
  - 季/年视图 title 里 "列数 = 星期数 × 7" 改为 "每行列数 = 最大显示星期 × 7"

### 9. 帮助按钮（主动重开欢迎弹窗）

- 顶部 stats panel 在 ✎ 和 ⚙ 之间加 `?` 帮助按钮
- 点击重新打开欢迎弹窗（不依赖 `dismissedIntro` 状态）
...
按当前实际功能/配置更新所有 tooltip 文字,把 v0.3.2/v0.3.3 时代过时的描述换掉。

**1. 视图切换按钮 title（动态）：**
- 旧：静态「月视图」「季度视图（3 个月）」「年视图（12 个月）」
- 新：根据当前视图动态生成"上一月/上一季（3 个月）/上一年"，按 `VIEW_STEP` 同步实际步长
- 新增 `updateNavTitles()` 函数,在 `setViewMode()` 和 `init()` 里调用
- 「今天」按钮 title：「今天」→「跳到今天」(更明确是导航动作)

**2. settings 菜单 4 个 tooltip 全部重写：**
- **星期数**（季/年）："季/年视图每行几周（1 周 = 7 列）。月视图固定 1 周不生效。范围 1-7，默认 5。"
- **方块大小上限**："日期格子最大边长（px）。所有视图通用 cap，大屏防格子过大。范围 40-200，默认 80。"
- **全屏**："开：日历撑满视口宽度（留 16px 边距）。关：由下方「日历宽度」滑条控制。竖屏强制开。"
- **日历宽度**："非全屏时手动调节日历面板宽度（1 周大小 ~ 视口 70%）。全屏开时该选项无效。"

**3. 底部 hint 替换：**
- 旧：「列数 21–28（cellSize 41–49px）效果最佳」(v0.3.2 时代的建议,现在滑条给自由度,这个约束没意义)
- 新：「日期格子永远正方形；方块上限 = 最大 cellSize，实际按面板宽度算。」(说明布局原理,更普适)

**4. 默认值保持不变（已符合业界默认布局）：**
- `maxStripCols = 35` (weekCount=5)：季/年 strip 视图 5 周一行（Google Calendar 月视图 6 周但 strip 视图常用 4-5）
- `maxCellSize = 80`：所有视图 cellSize cap（Cal Calendar / Fantastical 类应用常用 60-100）
- `fullscreen = true`：UI 占满视口（首次进入最直观）
- `calPanelWidth = null`：用户没拖过就一直自动算
